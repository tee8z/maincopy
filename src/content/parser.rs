...
pub fn validate_content<'source>(
    ...
    validate_tip_configuration(&publication, &post_candidates, &mut diagnostics);
...
struct PublicationCandidate {
    ...
    tips: Option<PublicationTipSettings>,
}
...
fn parse_publication_tips(
    ...
    let mut tips = match take_optional_table(table, "tips", "tips", path, diagnostics) {
        OptionalField::Missing => return Some(PublicationTipSettings::Unconfigured),
        OptionalField::Invalid => return None,
        OptionalField::Valid(value) => value,
    };
    let enabled = take_optional_bool(&mut tips, "enabled", "tips.enabled", path, diagnostics);
    let minimum = take_optional_u64(
        &mut tips,
        "minimum_sats",
        "tips.minimum_sats",
        path,
    ...
    if !enabled && minimum.is_missing() && maximum.is_missing() {
        return Some(PublicationTipSettings::Unconfigured);
    }
    ...
    match field {
        OptionalField::Missing => {
            diagnostics.push(ContentValidationError::new(
                ...
                "a configured tip policy requires both amount bounds",
            ));
            None
        }
        OptionalField::Invalid => None,
        OptionalField::Valid(value) => match TipAmount::new(value) {
...
fn parse_distribution(
    ...
    Some(DistributionSettings::new(x?))
}
    ...
    for post in posts {
        if post.tips == Some(PostTipPolicy::Enabled) {
            diagnostics.push(ContentValidationError::new(
                ...
                "post enables tips but the publication has no configured tip amount range",
            ));
        }
    }
}
...
fn take_optional_u64(
    ...
    match table.remove(key) {
        Some(Value::Integer(value)) => match u64::try_from(value) {
            Ok(value) => OptionalField::Valid(value),
            Err(_) => {
                diagnostics.push(ContentValidationError::new(
                    path.clone(),
                    field,
                    ContentValidationCode::TipAmountInvalid,
                    "tip amount must be a non-negative integer",
                ));
                OptionalField::Invalid
            }
        },
        Some(value) => {
            invalid_type(path, field, "integer", &value, diagnostics);
            OptionalField::Invalid
        }
        None => OptionalField::Missing,
    }
