...
- A closed authored-asset allowlist for passive image, audio, video, and font
  ...
  response.
- An inert default for renderer-generated bytes. A future Mermaid sanitizer
  must issue distinct trusted provenance before sanitized SVG can be served
  inline; extension alone never grants that capability.
