...
#[derive(Debug, Parser)]
#[command(name = "maincopyd", version, about = "Run the Maincopy server")]
pub(crate) struct ServerArguments {
    ...
    /// Override the private local admin socket or named-pipe endpoint.
    #[arg(long, value_name = "PATH")]
...
mod tests {
    ...
    #[test]
    fn server_uses_the_documented_default_host_file() {
        let arguments = ServerArguments::try_parse_from(["maincopyd"]).unwrap();
        let (path, _) = arguments.into_configuration();
        assert_eq!(path, PathBuf::from(DEFAULT_HOST_CONFIGURATION_FILE));
    }

    #[test]
    fn server_wires_every_typed_non_secret_override() {
    ...
    #[test]
    fn server_help_contains_no_secret_value_flags() {
        ...
        for expected in [
            ...
            assert!(help.contains(expected), "server help omitted {expected}");
        }
