...
///
/// ```compile_fail
/// use maincopy_server::payments::LexeProvider;
///
    ...
    async fn run_until_stop(
        ...
        run_bounded_queue(
            ...
            move |command| {
                ...
                async move {
                    execute_command(wallet, network, response_deadline, command).await;
                }
            },
        )
        .await
...
async fn execute_command(
    ...
    match command {
        LexeCommand::CreateTipInvoice {
            ...
            let result =
                execute_create(&wallet, network, response_deadline, request, sdk_request).await;
        ...
        LexeCommand::ReconcilePayment {
            ...
            let result =
                execute_payment_reconciliation(&wallet, network, response_deadline, request, index)
                    .await;
            let _ = respond_to.send(result);
        }
        LexeCommand::NextPaymentUpdates {
            ...
            let result =
                execute_next_payment_updates(&wallet, network, response_deadline, start_index)
                    .await;
            let _ = respond_to.send(result);
        }
    }
...
async fn execute_next_payment_updates(
    ...
    provider_update_poll(outcome, network)
}
