...
#[derive(Clone, Debug)]
pub(crate) struct ContentCandidateStore {
    root: PathBuf,
    limits: ContentTreeLimits,
}

#[derive(Debug, Eq, PartialEq)]
pub(crate) struct RetainedContentCandidate {
    pub(crate) digest: ContentTreeDigest,
...
impl ContentCandidateStore {
    ...
    pub(crate) fn load_all(
        &self,
    ) -> Result<Vec<RetainedContentCandidate>, ContentCandidateStoreError> {
...
#[derive(Debug, Error)]
pub(crate) enum ContentCandidateStoreError {
...
fn validate_asset_path(
    ...
    LogicalAssetPath::from_portable(portable).map_err(|_| {
