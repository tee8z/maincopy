...
async fn identifiers_and_hashes_use_blob_storage() {
    ...
    assert_eq!(
        ...
        [
            ...
            "mail_attempts.campaign_id:BLOB",
            "mail_attempts.configuration_binding:BLOB",
            "mail_attempts.enrollment_id:BLOB",
            "mail_attempts.feedback_source:BLOB",
            "mail_attempts.generation:BLOB",
            "mail_attempts.mail_epoch:BLOB",
            "mail_attempts.recipient_binding:BLOB",
            "mail_campaign_finishes.campaign_id:BLOB",
            "mail_campaign_finishes.command_fingerprint:BLOB",
            "mail_campaign_finishes.fence:BLOB",
            "mail_campaign_receipts.audit_event_id:BLOB",
            "mail_campaign_receipts.campaign_id:BLOB",
            "mail_campaign_receipts.command_fingerprint:BLOB",
            "mail_campaign_receipts.idempotency_key:BLOB",
            "mail_campaigns.campaign_id:BLOB",
            "mail_consent_resets.audit_event_id:BLOB",
            "mail_consent_resets.command_fingerprint:BLOB",
            "mail_consent_resets.idempotency_key:BLOB",
            "mail_consent_resets.new_epoch:BLOB",
            "mail_consent_resets.retired_epoch:BLOB",
            "mail_control_state.configuration_binding:BLOB",
            "mail_control_state.control_binding:BLOB",
            "mail_control_state.feedback_poll:BLOB",
            "mail_control_state.feedback_run:BLOB",
            "mail_control_state.feedback_source:BLOB",
            "mail_control_state.mail_epoch:BLOB",
            "mail_enrollments.enrollment_id:BLOB",
    ...
    for constraint in [
        ...
        "check(length(campaign_id)=16)",
        "check(length(attempt_id)=16)",
        "check(length(attempt_fence)=16)",
        "check(length(control_binding)=32)",
        "check(length(mail_epoch)=16)",
        "check(length(recipient_binding)=32)",
        "check(length(feedback_source)=32)",
        "check(length(enrollment_id)=16)",
        "check(length(generation)=16)",
        "check(length(retired_epoch)=16)",
        "check(length(new_epoch)=16)",
        "check(length(command_fingerprint)=32)",
        "check((nonce_digestisnull)=(nonce_expires_atisnull))",
        "check((feedback_pollisnull)=(feedback_poll_signed_atisnull))",
    ] {
        assert!(
