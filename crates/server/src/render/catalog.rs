...
impl ContentCatalog {
    ...
    /// Returns exact authored or renderer-generated bytes from the current candidate only.
    pub(crate) fn current_preview_asset(&self, path: &LogicalAssetPath) -> Option<Arc<[u8]>> {
        if let Some(asset) = self.local_assets.assets.get(path) {
            return Some(Arc::clone(&asset.bytes));
        }
        self.rendered_posts()
...
mod tests {
    ...
    fn retained_revision_keeps_its_exact_render_and_local_asset_store() {
        ...
        assert_eq!(
            retained_assets.assets.get(&cover).unwrap().bytes.as_ref(),
            b"old cover"
        );
        assert_eq!(
            current
    ...
    fn preview_asset_lookup_uses_current_authored_and_generated_bytes_only() {
        ...
        assert_eq!(
            catalog.current_preview_asset(&authored).unwrap().as_ref(),
            b"cover"
        );
